...
export default function StatsScreen({ questions, onHome }: Props) {
  ...
  return (
    <div className="min-h-dvh flex flex-col max-w-lg mx-auto px-5 py-6 gap-7">
      {/* Header */}
      <div className="flex items-center gap-3">
        <button
          onClick={onHome}
          className="text-sm font-medium text-stone-400 dark:text-stone-600 hover:text-stone-700 dark:hover:text-stone-300 transition-colors"
        >
          ←
        </button>
        <h2 className="text-xl font-bold text-stone-900 dark:text-stone-100">統計</h2>
      </div>

      {/* Summary numbers */}
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-stone-100 dark:bg-stone-900 rounded-xl px-4 py-4">
          <div className="text-2xl font-bold tabular-nums text-stone-900 dark:text-stone-100">
            {totalAttempts}
          </div>
          <div className="text-xs text-stone-400 mt-1">累計回答</div>
        </div>
        <div className="bg-stone-100 dark:bg-stone-900 rounded-xl px-4 py-4">
          ...
          <div className="text-xs text-stone-400 mt-1">正答率</div>
        </div>
        <div className="bg-stone-100 dark:bg-stone-900 rounded-xl px-4 py-4">
          <div className="text-2xl font-bold tabular-nums text-stone-700 dark:text-stone-300">
            {attempted.length}
          </div>
          <div className="text-xs text-stone-400 mt-1">/ {questions.length}問</div>
        </div>
      </div>

      {/* Last 7 days bar chart */}
      <div>
        <p className="text-xs font-semibold text-stone-400 dark:text-stone-600 uppercase tracking-wider mb-4">
          直近7日間
        </p>
          ...
          {sessions.map((s) => (
            <div key={s.date} className="flex-1 flex flex-col items-center gap-1.5">
              <div className="w-full flex flex-col-reverse" style={{ height: '64px' }}>
                <div
                  className="w-full bg-stone-400 dark:bg-stone-600 rounded-sm transition-all"
                  style={{
      ...
      {worst10.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-stone-400 dark:text-stone-600 uppercase tracking-wider mb-3">
          ...
          <div className="divide-y divide-stone-100 dark:divide-stone-900">
            {worst10.map(({ q, rate, h }, i) => (
              <div key={q.id} className="flex items-center gap-3 py-2.5">
                <div className="text-xs text-stone-300 dark:text-stone-700 w-4 shrink-0 tabular-nums">
                  {i + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-stone-800 dark:text-stone-200 truncate">{q.ja}</div>
                  <div className="text-xs text-stone-400 mt-0.5">{q.chapter} L{q.lesson} · {q.theme}</div>
                </div>
                <div className="shrink-0 text-right">
                  <div className="text-sm font-bold text-rose-500 tabular-nums">{Math.round(rate * 100)}%</div>
                  <div className="text-xs text-stone-400 tabular-nums">{h.totalAttempts}回</div>
                </div>
              </div>
            ))}
          </div>
            ...
            .map(([key, stats]) => {
              ...
              return (
                <div key={key} className="flex items-center gap-3">
                  <div className="text-sm text-stone-600 dark:text-stone-400 w-20 shrink-0 tabular-nums">
                    {key}
                  </div>
                  ...
                  <div className="text-sm font-medium text-stone-500 dark:text-stone-400 w-10 text-right tabular-nums">
                    {rate !== null ? `${rate}%` : '—'}
                  </div>
                </div>
              );
      ...
      <div className="pb-4">
        <button
          onClick={handleReset}
          className="text-xs text-stone-300 dark:text-stone-700 hover:text-rose-500 dark:hover:text-rose-500 transition-colors underline"
        >
          データをリセット
        </button>
      </div>
